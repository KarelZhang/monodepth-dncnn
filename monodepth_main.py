...
def train(params):
    ...
    with tf.Graph().as_default(), tf.device('/cpu:0'):
        ...
        var_dncnn_restore = [val for val in var if 'block' in val.name]
        print(var_dncnn_restore)

        dncnn_saver = tf.train.Saver(var_dncnn_restore)
        dncnn_saver.restore(sess, 'checkpoint_demo/model-DnCNN-tensorflow-67336')
...
def test(params):
    ...
    for step in range(num_test_samples):
        ...
        plt.imsave(os.path.join("{}_disp.png".format("test_tra_"+str(step))), disp[0].squeeze(), cmap='plasma')
    print('done.')
